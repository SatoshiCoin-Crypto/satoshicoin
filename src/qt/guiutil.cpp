...
{
    // Convert Satoshi:// to Satoshi:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Satoshi://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Satoshi:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Satoshi.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Satoshi.lnk
...
{
    return GetAutostartDir() / "Satoshi.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Satoshi.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Satoshi\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Satoshi app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Satoshi app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Satoshi-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Satoshi-qt [" + tr("command-line options") + "]                     " + "\n";
