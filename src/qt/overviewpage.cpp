...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSatoshiAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSatoshiAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSatoshiAmount = strAnonymizeSatoshiAmount.remove(strAnonymizeSatoshiAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSatoshiAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSatoshiAmount*COIN) nMaxToAnonymize = nAnonymizeSatoshiAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSatoshiAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSatoshiAmount));
        strAnonymizeSatoshiAmount = strAnonymizeSatoshiAmount.remove(strAnonymizeSatoshiAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSatoshiAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
