...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSatoshiAmount"))
        settings.setValue("nAnonymizeSatoshiAmount", 1000);
    nAnonymizeSatoshiAmount = settings.value("nAnonymizeSatoshiAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSatoshiAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSatoshiAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSatoshiAmount:
            return QVariant(nAnonymizeSatoshiAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSatoshiAmount:
            nAnonymizeSatoshiAmount = value.toInt();
            settings.setValue("nAnonymizeSatoshiAmount", nAnonymizeSatoshiAmount);
            emit AnonymizeSatoshiAmountChanged(nAnonymizeSatoshiAmount);
            break;
        default:
            break;
        }
