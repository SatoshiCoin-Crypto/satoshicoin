...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Satoshi.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Satoshid.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeSatoshiamount=<n> " + _("Keep N Satoshi anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Satoshi is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Satoshi is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Satoshi version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Satoshi server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Satoshi") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Satoshi to complete") << "\n";
    ...
    nAnonymizeSatoshiAmount = GetArg("-anonymizeSatoshiamount", 0);
    if(nAnonymizeSatoshiAmount > 999999) nAnonymizeSatoshiAmount = 999999;
    if(nAnonymizeSatoshiAmount < 2) nAnonymizeSatoshiAmount = 2;
    ...
    LogPrintf("Anonymize Satoshi Amount %d\n", nAnonymizeSatoshiAmount);
